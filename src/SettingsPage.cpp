...
{
    // FIX 6 & 7: Validate relay URL and code length before saving.

    QString relay = relayEdit->text().trimmed();

    // If a relay URL is provided, it must be ws:// or wss:// only.
    // This prevents someone entering a malicious URL that could be used
    // to redirect transfers to an attacker-controlled server.
    if (!relay.isEmpty()) {
        QUrl parsed(relay);
        if (parsed.scheme() != "ws" && parsed.scheme() != "wss") {
            saveBtn->setText("⚠ Relay must be ws:// or wss://");
            QTimer::singleShot(2000, this, [this]{ saveBtn->setText("Save settings"); });
            return;
        }
    }

    // Code length must be a sensible number between 2 and 10.
    // This prevents garbage values from being passed to the wormhole CLI.
    int codeLen = codeLengthEdit->text().toInt();
    if (codeLen < 2 || codeLen > 10) {
        saveBtn->setText("⚠ Code length must be 2–10");
        QTimer::singleShot(2000, this, [this]{ saveBtn->setText("Save settings"); });
        return;
    }

    QSettings s;
    s.setValue("relay", relay);
    s.setValue("codeLength", codeLen);
